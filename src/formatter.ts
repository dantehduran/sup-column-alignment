...
export default class Formatter {
			...
			get: function (key: any, defaultValue?: any): any {
				if (langConfig) {
					...
					if (langConfig.hasOwnProperty(key1)) {
						return langConfig[key1 as keyof typeof langConfig];
					}
				}
		...
		while (pos < text.length) {
			...
			if (currTokenType == TokenType.Block) {
				...
				while (pos < text.length) {
					...
					if (bracket == char) {
						++bracketCount;
					} else if (bracket == BRACKET_PAIR[char as keyof typeof BRACKET_PAIR] && text.charAt(pos - 1) != '\\') {
						if (bracketCount == 1) {
							break;
						} else {
							--bracketCount;
	...
	protected arrayAnd(array1: TokenType[], array2: TokenType[]): TokenType[] {
		...
		for (var i = 0; i < array2.length; ++i) {
			if (map[array2[i] as keyof typeof map]) {
				res.push(array2[i]);
			}
		}
		return res;
	...
	protected format(range: LineRange): string[] {
		...
		const configSTT = configWS[stt] || configDef[stt as keyof typeof configDef];
